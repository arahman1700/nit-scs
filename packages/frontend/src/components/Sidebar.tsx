...
import {
  ...
  LayoutDashboard, Warehouse, PackageCheck, Send, ShieldCheck, Truck,
  Database, Settings, Package, RotateCcw, ClipboardList, Users,
  PlusCircle, FileText, Briefcase, CheckSquare, ListTodo, Ship,
  DoorOpen, Search, AlertTriangle,
} from 'lucide-react';
...
const NavItemComponent: React.FC<{ item: NavItemWithIcon; isOpen: boolean; isActive: (path?: string) => boolean }> = ({ item, isOpen, isActive }) => {
    ...
    <li className="mb-2">
      <div className={`flex flex-col transition-all duration-200`}>
        <Link
          ...
          `}
        >
          {active && <div className="absolute left-0 top-0 bottom-0 w-1 bg-nesma-secondary shadow-[0_0_10px_#80D1E9]"></div>}

          {Icon && <Icon size={22} className={`min-w-[22px] z-10 transition-transform duration-300 ${active ? 'text-nesma-secondary scale-110' : 'group-hover:scale-110'}`} />}

          <div className={`flex-1 flex justify-between items-center overflow-hidden z-10 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 lg:hidden'}`}>
             <span className={`font-medium whitespace-nowrap text-sm tracking-wide ${active ? 'text-white' : ''}`}>{item.label}</span>
             {hasChildren && (
            ...
            {(item.children as NavItemWithIcon[])!.map((child, idx) => (
              ...
              >
                {child.type === 'divider' ? (
                  ...
                  <>
                    <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${isActive(child.path) ? 'bg-nesma-secondary shadow-[0_0_5px_#80D1E9]' : 'bg-gray-600'}`}></div>
...
export const Sidebar: React.FC<SidebarProps> = ({ role, isOpen, setRole, onLogout }) => {
  ...
  const isLinkActive = (path?: string) => {
    ...
    if (!queryPart && pathPart !== '/' && !['/admin', '/warehouse', '/transport', '/engineer'].includes(pathPart) && location.pathname.startsWith(pathPart)) return true;
    return false;
  };

  return (
    <aside className={`${isOpen ? 'w-80' : 'w-24 hidden lg:flex'} glass-panel transition-all duration-500 ease-in-out flex flex-col z-50 h-full border-r border-white/5 bg-[#051020]/95 backdrop-blur-xl shadow-2xl`}>
      {/* Logo Area */}
        ...
        {isOpen ? (
          <div className="z-10 flex flex-col items-center animate-fade-in transform hover:scale-105 transition-transform duration-300 w-full">
             ...
             <div className="flex items-center gap-2 mt-1">
               ...
               <span className="text-[9px] tracking-[0.2em] text-nesma-secondary font-bold uppercase whitespace-nowrap">Supply Chain</span>
               <div className="h-px w-6 bg-gradient-to-l from-transparent to-nesma-secondary/50"></div>
             </div>
          </div>
        ) : (
          <div className="font-black text-3xl text-transparent bg-clip-text bg-gradient-to-br from-white to-nesma-secondary tracking-widest z-10">N</div>
        )}
      </div>

      {/* Navigation */}
      <div className="flex-1 overflow-hidden flex flex-col px-3 pt-4">
        <p className={`text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-4 transition-opacity duration-300 px-2 ${!isOpen && 'text-center opacity-0 lg:hidden'}`}>
          Main Menu
        </p>
      ...
      <div className="p-4 border-t border-white/5 bg-black/20 backdrop-blur-md">
        {isOpen ? (
          <div className="mb-3 bg-[#0E2841] p-3 rounded-xl border border-white/5 shadow-inner">
            <label className="text-[10px] text-gray-400 block mb-1.5 font-bold uppercase tracking-wide">Current Persona</label>
            <select
              value={role}
            ...
            >
             ...
             <div className="flex justify-center mb-4">
                 <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-xs font-bold text-nesma-secondary border border-white/10" title={role}>
                     {role.charAt(0)}
                 </div>
             </div>
        )}
