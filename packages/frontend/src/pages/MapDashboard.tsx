...
export const MapDashboard: React.FC = () => {
  ...
  const locations = useMemo(() => {
    ...
    type RawWarehouse = Warehouse & { warehouseName?: string; warehouseCode?: string; latitude?: number; longitude?: number };
    ...
    type RawProject = Project & { projectName?: string; projectCode?: string; latitude?: number; longitude?: number; lat?: number; lng?: number };
  ...
  const filteredLocations = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(loc =>
        loc.name.toLowerCase().includes(term) || loc.details.toLowerCase().includes(term)
      );
    }
    return result;
    ...
    <div className="flex gap-6 h-[calc(100vh-140px)] animate-fade-in">
      ...
      <div className="w-80 glass-card rounded-2xl flex flex-col overflow-hidden hidden lg:flex border border-white/10 shadow-2xl">
                   ...
                   <div className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                         <div className="flex justify-between items-start">
                            ...
                            <span className={`text-[10px] px-1.5 rounded flex-shrink-0 ml-1 ${selectedLocation?.id === loc.id ? 'bg-nesma-primary text-white' : 'bg-white/10 text-gray-400'}`}>{loc.type}</span>
                         </div>
        ...
        <div className="p-4 border-t border-white/10 text-xs text-gray-500 text-center bg-black/20">
            {isLoading ? 'Loading...' : `${filteredLocations.length} location${filteredLocations.length !== 1 ? 's' : ''} in Kingdom`}
        </div>
      </div>
