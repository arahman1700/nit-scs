...
export const StockTransferForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<any>({ transferType: 'inter_warehouse' });
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] glass-card rounded-2xl p-8 text-center animate-fade-in mx-auto max-w-2xl mt-10 border border-green-500/30 bg-gradient-to-b from-green-900/10 to-transparent">
        ...
        <p className="text-gray-400 mb-6">Transfer <span className="text-nesma-secondary font-medium">{documentNumber}</span> — {totalValue.toLocaleString()} SAR</p>
        <div className="flex gap-4">
          <button onClick={() => { reset(); setFormData({ transferType: 'inter_warehouse' }); setLineItems([]); }} className="px-6 py-3 border border-white/20 rounded-xl text-gray-300 hover:bg-white/10 transition-all">Create Another</button>
          <button onClick={() => navigate('/admin/warehouse/stock-transfer')} className="px-6 py-3 bg-nesma-primary border border-nesma-primary/50 text-white rounded-xl transition-all">View All Transfers</button>
        </div>
      </div>
    );
  }
      ...
      <div className="glass-card rounded-2xl overflow-hidden shadow-2xl border border-white/10">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-8">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-gray-300 ml-1">Transfer Type <span className="text-red-400">*</span></label>
                <select value={formData.transferType} onChange={(e) => handleChange('transferType', e.target.value)} className="nesma-input px-4 py-3 w-full bg-white/5 border border-white/10 rounded-xl text-white focus:border-nesma-secondary outline-none">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-gray-300 ml-1">From Warehouse <span className="text-red-400">*</span></label>
                <select onChange={(e) => handleChange('fromWarehouse', e.target.value)} required className="nesma-input px-4 py-3 w-full bg-white/5 border border-white/10 rounded-xl text-white focus:border-nesma-secondary outline-none">
                  <option value="">Select...</option>
                  {warehouses.map(w => <option key={w.id as string} value={w.name as string}>{w.name as string}</option>)}
                </select>
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-gray-300 ml-1">To Warehouse <span className="text-red-400">*</span></label>
                <select onChange={(e) => handleChange('toWarehouse', e.target.value)} required className="nesma-input px-4 py-3 w-full bg-white/5 border border-white/10 rounded-xl text-white focus:border-nesma-secondary outline-none">
                  <option value="">Select...</option>
                  {warehouses.map(w => <option key={w.id as string} value={w.name as string}>{w.name as string}</option>)}
                </select>
              </div>
              {formData.transferType === 'inter_project' && (
                <>
                  <div className="flex flex-col gap-2">
                    ...
                    <select onChange={(e) => handleChange('fromProject', e.target.value)} className="nesma-input px-4 py-3 w-full bg-white/5 border border-white/10 rounded-xl text-white focus:border-nesma-secondary outline-none">
                      <option value="">Select...</option>
                      {projects.map(p => <option key={p.id as string} value={p.name as string}>{p.name as string}</option>)}
                    </select>
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium text-gray-300 ml-1">To Project</label>
                    <select onChange={(e) => handleChange('toProject', e.target.value)} className="nesma-input px-4 py-3 w-full bg-white/5 border border-white/10 rounded-xl text-white focus:border-nesma-secondary outline-none">
                      <option value="">Select...</option>
                      {projects.map(p => <option key={p.id as string} value={p.name as string}>{p.name as string}</option>)}
                    </select>
                  </div>
          ...
          <div className="pt-8 border-t border-white/10 flex justify-between items-center">
            {totalValue > 0 && (
              <div className="text-sm text-gray-400">Total Value: <span className="text-nesma-secondary font-bold text-lg">{totalValue.toLocaleString()} SAR</span></div>
            )}
