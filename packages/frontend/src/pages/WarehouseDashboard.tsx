...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${colorMap[status] || 'bg-gray-500/10 text-gray-400 border-gray-500/20'}`}>
      {status}
    </span>
  );
};
...
export const WarehouseDashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (tab) {
      const mappedTab = tab === 'mrrv' ? 'receive' : tab === 'mirv' ? 'issue' : tab === 'mrv' ? 'return' : tab as typeof activeTab;
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      {isLoading && (
        <div className="space-y-4">
          {[1,2,3].map(i => <div key={i} className="animate-pulse bg-white/5 rounded h-8 w-full"></div>)}
        </div>
      )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="glass-card p-6 rounded-xl">
              <h3 className="font-bold text-lg text-white mb-4 border-b border-white/10 pb-2 flex items-center justify-between">
                ...
                <button onClick={() => handleTabChange('receive')} className="text-xs text-nesma-secondary hover:text-white flex items-center gap-1">
                  View All <ChevronRight size={12} />
                </button>
              </h3>
              <div className="space-y-3">
                {mrrvData.slice(0, 3).map(mrrv => (
                  <div key={mrrv.id as string} className="flex justify-between items-center p-3 bg-white/5 rounded-xl hover:bg-white/10 cursor-pointer transition-colors border border-transparent hover:border-emerald-500/30 group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{mrrv.supplier as string} • {mrrv.date as string}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-nesma-secondary font-medium">{Number(mrrv.value || 0).toLocaleString()} SAR</span>
            ...
            <div className="glass-card p-6 rounded-xl">
              <h3 className="font-bold text-lg text-white mb-4 border-b border-white/10 pb-2 flex items-center justify-between">
                ...
                <button onClick={() => handleTabChange('issue')} className="text-xs text-nesma-secondary hover:text-white flex items-center gap-1">
                  View All <ChevronRight size={12} />
                </button>
              </h3>
              <div className="space-y-3">
                {mirvData.slice(0, 3).map(mirv => (
                  <div key={mirv.id as string} className="flex justify-between items-center p-3 bg-white/5 rounded-xl hover:bg-white/10 cursor-pointer transition-colors border border-transparent hover:border-blue-500/30 group">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{mirv.project as string} • {mirv.requester as string}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-nesma-secondary font-medium">{Number(mirv.value || 0).toLocaleString()} SAR</span>
            ...
            <div className="glass-card p-6 rounded-xl border border-red-500/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {lowStockItems.map((item) => (
                  <div key={item.id as string} className="flex items-center justify-between p-3 bg-red-500/5 rounded-xl border border-red-500/10">
                    <div>
                      <p className="text-sm font-medium text-gray-200">{item.name as string}</p>
                      <p className="text-xs text-gray-500">{item.code as string} • {item.warehouse as string}</p>
                    </div>
                    <div className="text-right">
                      <p className={`text-lg font-bold ${item.quantity === 0 ? 'text-red-400' : 'text-amber-400'}`}>{item.quantity as number}</p>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Link
              ...
              onClick={(e) => { e.preventDefault(); navigate('/admin/forms/mrrv'); }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Approved', count: mrrvData.filter(m => m.status === 'Approved').length, color: 'text-emerald-400' },
              { label: 'Inspected', count: mrrvData.filter(m => m.status === 'Inspected').length, color: 'text-blue-400' },
            ].map((stat, i) => (
              <div key={i} className="glass-card p-4 rounded-xl text-center hover:bg-white/5 transition-all cursor-pointer">
                ...
                <tbody className="divide-y divide-white/5 text-sm">
                  {mrrvData.map(mrrv => (
                    <tr key={mrrv.id as string} className="hover:bg-white/5 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-white font-medium">{Number(mrrv.value || 0).toLocaleString()} SAR</td>
                      <td className="px-6 py-4 text-gray-400 font-mono text-xs">{(mrrv.poNumber as string) || '-'}</td>
                      <td className="px-6 py-4">
                        {mrrv.rfimCreated ? (
                          <span className="text-emerald-400 flex items-center gap-1 text-xs"><CheckCircle size={12} /> Created</span>
                        ) : mrrv.rfimRequired ? (
                          <span className="text-amber-400 flex items-center gap-1 text-xs"><Clock size={12} /> Required</span>
                        ) : (
                          <span className="text-gray-600 text-xs">-</span>
                        )}
                      </td>
                      <td className="px-6 py-4"><StatusBadge status={mrrv.status as string} /></td>
                      <td className="px-6 py-4">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              ...
              { label: 'Pending', count: mirvData.filter(m => m.status === 'Pending Approval').length, color: 'text-amber-400' },
              { label: 'Approved', count: mirvData.filter(m => m.status === 'Approved').length, color: 'text-emerald-400' },
                ...
                <tbody className="divide-y divide-white/5 text-sm">
                  {mirvData.map(mirv => (
                    <tr key={mirv.id as string} className="hover:bg-white/5 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-gray-200 max-w-[200px] truncate" title={mirv.project as string}>{mirv.project as string}</td>
                      ...
                      <td className="px-6 py-4 text-white font-medium">{Number(mirv.value || 0).toLocaleString()} SAR</td>
                      ...
                      <td className="px-6 py-4"><StatusBadge status={mirv.status as string} /></td>
                      <td className="px-6 py-4">
                        <button className="p-1.5 rounded-lg hover:bg-white/10 text-gray-500 hover:text-nesma-secondary opacity-0 group-hover:opacity-100 transition-all">
                          <Eye size={16} />
                        </button>
      ...
      {activeTab === 'return' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {mrvData.map(mrv => (
              <div key={mrv.id as string} className="glass-card p-6 rounded-xl hover:border-nesma-secondary/30 transition-all group cursor-pointer">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <span className={`text-sm font-medium ${
                      mrv.returnType === 'Surplus' ? 'text-blue-400' :
                      mrv.returnType === 'Damaged' ? 'text-red-400' :
                      mrv.returnType === 'Wrong Item' ? 'text-amber-400' :
                      'text-emerald-400'
                    }`}>{mrv.returnType as string}</span>
                  </div>
                  <div>
                ...
                {filteredInventory.map((item) => {
                  ...
                  return (
                    <tr key={item.id as string} className="hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-6 py-4 font-bold text-nesma-secondary">{(item.quantity as number)?.toLocaleString()}</td>
