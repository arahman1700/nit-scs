...
import { Package, DollarSign, AlertTriangle, Warehouse as WarehouseIcon, ArrowDownCircle, ArrowRightLeft } from 'lucide-react';
...
function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    Active: 'bg-emerald-500/20 text-emerald-400', Available: 'bg-emerald-500/20 text-emerald-400',
    'Low Stock': 'bg-amber-500/20 text-amber-400', Pending: 'bg-amber-500/20 text-amber-400',
    'Out of Stock': 'bg-red-500/20 text-red-400', Approved: 'bg-emerald-500/20 text-emerald-400',
    'In Transit': 'bg-blue-500/20 text-blue-400', Completed: 'bg-emerald-500/20 text-emerald-400',
  };
  return (
...
const Spinner = () => <div className="flex justify-center py-12"><div className="w-8 h-8 border-2 border-nesma-secondary border-t-transparent rounded-full animate-spin" /></div>;
...
export const InventorySectionPage: React.FC = () => {
  ...
  const kpis: KpiCardProps[] = [
    ...
    { title: 'Inventory Value', value: `SAR ${(inv?.totalValue ?? 0).toLocaleString()}`, icon: DollarSign, color: 'bg-emerald-500' },
    { title: 'Low Stock Alerts', value: inv?.lowStock ?? 0, icon: AlertTriangle, color: 'bg-red-500', alert: (inv?.lowStock ?? 0) > 0 },
            ...
            <div className="glass-card p-6 rounded-xl">
              ...
              {byCategory.length > 0 ? (
                <ResponsiveContainer width="100%" height={260}>
                  <PieChart>
                    <Pie data={byCategory} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label={renderPieLabel}>
                      {byCategory.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                    </Pie>
                    <Tooltip contentStyle={{ background: '#0a1628', border: '1px solid rgba(255,255,255,.1)', borderRadius: 8, color: '#fff' }} />
                  </PieChart>
                </ResponsiveContainer>
        ...
        'stock-levels': (
          <div className="glass-card rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-white/10">
              ...
              <button onClick={() => navigate('/admin/inventory/inventory')} className="text-nesma-secondary text-xs hover:underline">View All</button>
            </div>
            <table className="w-full">
              <thead><tr className="border-b border-white/10">
                {['Code', 'Name', 'Warehouse', 'Qty', 'Status'].map(h => <th key={h} className="text-nesma-secondary text-xs uppercase tracking-wider text-left px-4 py-3">{h}</th>)}
              </tr></thead>
              <tbody>
                {inventoryRows.map((r) => (
                  <tr key={r.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="px-4 py-3"><StatusBadge status={r.stockStatus ?? 'Active'} /></td>
                  </tr>
                ))}
                {inventoryRows.length === 0 && <tr><td colSpan={5} className="text-center text-gray-500 py-8">No items</td></tr>}
              </tbody>
            </table>
          </div>
        ),
        warehouses: (
          <div className="glass-card rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-white/10">
              ...
              <button onClick={() => navigate('/admin/inventory/warehouses')} className="text-nesma-secondary text-xs hover:underline">View All</button>
            </div>
            <table className="w-full">
              <thead><tr className="border-b border-white/10">
                {['Name', 'City', 'Type', 'Status'].map(h => <th key={h} className="text-nesma-secondary text-xs uppercase tracking-wider text-left px-4 py-3">{h}</th>)}
              </tr></thead>
              <tbody>
                {warehouseRows.map(r => (
                  <tr key={r.id} className="border-b border-white/5 hover:bg-white/5">
                ...
                {warehouseRows.length === 0 && <tr><td colSpan={4} className="text-center text-gray-500 py-8">No warehouses</td></tr>}
              </tbody>
            </table>
          </div>
        ),
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="glass-card rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between p-4 border-b border-white/10">
                ...
                <button onClick={() => navigate('/admin/logistics/gate-pass')} className="text-nesma-secondary text-xs hover:underline">View All</button>
              </div>
              <table className="w-full">
                <thead><tr className="border-b border-white/10">
                  {['ID', 'Type', 'Date', 'Status'].map(h => <th key={h} className="text-nesma-secondary text-xs uppercase tracking-wider text-left px-4 py-3">{h}</th>)}
                </tr></thead>
                <tbody>
                  {gatePassRows.map(r => (
                  ...
                  {gatePassRows.length === 0 && <tr><td colSpan={4} className="text-center text-gray-500 py-8">No gate passes</td></tr>}
                </tbody>
              </table>
            </div>
            <div className="glass-card rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between p-4 border-b border-white/10">
                ...
                <button onClick={() => navigate('/admin/logistics/stock-transfer')} className="text-nesma-secondary text-xs hover:underline">View All</button>
              </div>
              <table className="w-full">
                <thead><tr className="border-b border-white/10">
                  {['ID', 'From', 'To', 'Status'].map(h => <th key={h} className="text-nesma-secondary text-xs uppercase tracking-wider text-left px-4 py-3">{h}</th>)}
                </tr></thead>
                <tbody>
                  ...
                  {stockTransferRows.length === 0 && <tr><td colSpan={4} className="text-center text-gray-500 py-8">No transfers</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ),
        dashboard: <Suspense fallback={<Spinner />}><InventoryDashboard /></Suspense>,
        shifting: <Suspense fallback={<Spinner />}><ShiftingMaterialDashboard /></Suspense>,
        'non-moving': <Suspense fallback={<Spinner />}><NonMovingMaterialsDashboard /></Suspense>,
      }}
    />
  );
};
