  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] glass-card rounded-2xl p-8 text-center animate-fade-in mx-auto max-w-2xl mt-10 border border-green-500/30 bg-gradient-to-b from-green-900/10 to-transparent">
        ...
        <p className="text-gray-400 mb-2">MRF <span className="text-nesma-secondary font-medium">{documentNumber}</span></p>
        <p className="text-gray-500 text-sm mb-6">Will convert to MIRV upon approval. Approval: {approvalLevel.label}</p>
        <div className="flex gap-4">
          <button onClick={() => { reset(); setFormData({}); setLineItems([]); }} className="px-6 py-3 border border-white/20 rounded-xl text-gray-300 hover:bg-white/10 transition-all">Create Another</button>
          <button onClick={() => navigate('/admin')} className="px-6 py-3 bg-nesma-primary border border-nesma-primary/50 text-white rounded-xl transition-all">Dashboard</button>
        </div>
      </div>
    );
  }
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-gray-300 ml-1">Project <span className="text-red-400">*</span></label>
                <select onChange={(e) => handleChange('project', e.target.value)} required className="nesma-input px-4 py-3 w-full bg-white/5 border border-white/10 rounded-xl text-white focus:border-nesma-secondary outline-none">
              ...
              <div className="flex flex-col gap-2">
                ...
                <select onChange={(e) => handleChange('warehouse', e.target.value)} className="nesma-input px-4 py-3 w-full bg-white/5 border border-white/10 rounded-xl text-white focus:border-nesma-secondary outline-none">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-medium text-gray-300 ml-1">Urgency <span className="text-red-400">*</span></label>
                <select onChange={(e) => handleChange('urgency', e.target.value)} required className="nesma-input px-4 py-3 w-full bg-white/5 border border-white/10 rounded-xl text-white focus:border-nesma-secondary outline-none">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input type="text" value="Current User" readOnly className="nesma-input px-4 py-3 w-full bg-white/5 border border-white/10 rounded-xl text-gray-400 outline-none cursor-not-allowed" />
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-sm font-medium text-gray-300 ml-1">Notes</label>
              <textarea onChange={(e) => handleChange('notes', e.target.value)} placeholder="Reason for request, special requirements..." className="nesma-input px-4 py-3 w-full min-h-[100px] bg-white/5 border border-white/10 rounded-xl text-white focus:border-nesma-secondary outline-none" />
            </div>
          </div>
            ...
            <div className="glass-card rounded-xl p-4 border border-white/10 bg-gradient-to-r from-white/5 to-transparent space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-gray-300">Required Approval: <span className={`font-medium ${totalValue >= 100000 ? 'text-orange-400' : totalValue >= 50000 ? 'text-yellow-400' : 'text-green-400'}`}>{approvalLevel.label}</span></span>
                </div>
                <span className="text-nesma-secondary font-bold text-lg">{totalValue.toLocaleString()} SAR</span>
              </div>
              <p className="text-xs text-gray-500">Upon approval, this MRF will auto-convert to a MIRV (Material Issue Request)</p>
            </div>
          )}
